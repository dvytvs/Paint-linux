...
interface ToolbarProps {
  ...
  activeColorSlot: 1 | 2;
  setActiveColorSlot: (slot: 1 | 2) => void;
  strokeSize: number;
...
export default function Toolbar({
  ...
  activeColorSlot, setActiveColorSlot,
  ...
  return (
    <div className="h-28 bg-[#202020] border-b border-black/40 flex flex-nowrap items-start px-2 py-2 gap-1 shadow-sm z-50 relative text-gray-300 select-none overflow-x-auto overflow-y-hidden flex-shrink-0 w-full scrollbar-thin scrollbar-thumb-gray-600">
      ...
      <div className="flex flex-col h-full px-3 border-r border-white/10 flex-shrink-0">
        <div className="flex gap-2 h-[60px] items-center">
          <div className="grid grid-cols-7 gap-0.5 bg-[#1a1a1a] border border-white/10 rounded p-1 w-48 h-[52px] overflow-y-auto">
            ...
            <ToolBtn icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 10 l6 -6 l10 4 l-4 12 l-10 -2 Z"/></svg>} active={currentTool==='polygon'} onClick={()=>setCurrentTool('polygon')} label={t('tool.polygon')} />
      ...
      <div className="flex flex-col h-full px-3 border-r border-white/10 flex-shrink-0">
        <div className="flex gap-3 h-[60px] items-center">
          ...
          <div className="flex flex-col gap-1 ml-1">
            <label className="w-6 h-6 rounded-full border border-white/20 flex items-center justify-center hover:bg-white/10 cursor-pointer overflow-hidden relative" title={t('ui.colors')}>
               <div className="absolute inset-0 bg-gradient-to-tr from-purple-500 via-red-500 to-yellow-500"></div>
               <input 
                 type="color" 
