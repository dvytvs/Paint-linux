...
export default function DrawingCanvas({ currentTool, setCurrentTool, primaryColor, secondaryColor, strokeSize, width, height, canvasRef, onDraw, onColorPick, pastedImage, showRulers, showGridlines }: CanvasProps) {
  ...
  const [textInput, setTextInput] = useState<{x: number, y: number, w: number, h: number, text: string, isCreating?: boolean, isMoving?: boolean, isResizing?: string | null} | null>(null);
  ...
  const [systemFonts, setSystemFonts] = useState<{family: string, fullName: string}[]>([]);
    ...
    const loadFonts = async () => {
      try {
        if ('queryLocalFonts' in window) {
           ...
           const unique = Array.from(new Set(fonts.map((f: any) => f.family))).map((f: any) => ({
             family: f as string, fullName: f as string
         ...
         setSystemFonts([
           ...
           {family: 'Comic Sans MS', fullName: 'Comic Sans MS'},
           {family: 'Impact', fullName: 'Impact'},
  ...
  }, [currentTool, curveState, polygonState, textInput]);
    ...
    if (overlay) {
      ...
      if (ctx) {
        ...
        if (selection) {
          ...
          // Отрисовка path для lasso-select (только в процессе рисования)
          if (selection.path && selection.path.length > 0 && !selection.active) {
            ctx.setLineDash([5, 5]);
            ...
            ctx.stroke();
          } else if (selection.active || !selection.path) {
  ...
  const commitText = () => {
    if (textInput && textInput.text.trim() && canvasRef.current) {
      const ctx = canvasRef.current.getContext('2d');
      if (ctx) {
        let fontString = '';
        if (textSettings.italic) fontString += 'italic ';
        if (textSettings.bold) fontString += 'bold ';
        fontString += `${textSettings.fontSize}px "${textSettings.fontFamily}"`;
        
        ctx.font = fontString;
        ctx.fillStyle = primaryColor;
        ctx.textBaseline = 'top';
        
        const maxWidth = Math.abs(textInput.w) - 8;
        const paragraphs = textInput.text.split('\n');
        const lines: string[] = [];
        ...
        lines.forEach((line, i) => {
          const ly = textInput.y + 4 + i * (textSettings.fontSize * 1.2);
          ctx.fillText(line, textInput.x + 4, ly);
          
          if (textSettings.underline || textSettings.strikethrough) {
            const metrics = ctx.measureText(line);
            ctx.beginPath();
            if (textSettings.underline) {
              ctx.moveTo(textInput.x + 4, ly + textSettings.fontSize);
              ctx.lineTo(textInput.x + 4 + metrics.width, ly + textSettings.fontSize);
            }
            if (textSettings.strikethrough) {
              const sy = ly + textSettings.fontSize / 2;
              ctx.moveTo(textInput.x + 4, sy);
              ctx.lineTo(textInput.x + 4 + metrics.width, sy);
            }
            ctx.strokeStyle = primaryColor;
            ctx.lineWidth = Math.max(1, Math.floor(textSettings.fontSize / 15));
            ctx.stroke();
          }
        });
  ...
  const startDrawing = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (currentTool === 'text') {
      setIsDrawing(true);
      setStartPos({x, y});
      setTextInput({x, y, w: 0, h: 0, text: '', isCreating: true});
      return;
    }
    ...
    if (currentTool === 'select' || currentTool === 'lasso-select') {
      if (selection && selection.active) {
        const handleSize = 20; // much larger hit area
        const hs = handleSize / 2;
        const handles = [
          { id: 'nw', x: selection.x - hs, y: selection.y - hs },
          { id: 'n', x: selection.x + selection.w/2 - hs, y: selection.y - hs },
          { id: 'ne', x: selection.x + selection.w - hs, y: selection.y - hs },
          { id: 'w', x: selection.x - hs, y: selection.y + selection.h/2 - hs },
          { id: 'e', x: selection.x + selection.w - hs, y: selection.y + selection.h/2 - hs },
          { id: 'sw', x: selection.x - hs, y: selection.y + selection.h - hs },
          { id: 's', x: selection.x + selection.w/2 - hs, y: selection.y + selection.h - hs },
          { id: 'se', x: selection.x + selection.w - hs, y: selection.y + selection.h - hs },
        ];
  ...
  const draw = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (currentTool === 'text' && textInput?.isCreating && isDrawing && startPos) {
  ...
  const stopDrawing = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (currentTool === 'text' && textInput?.isCreating) {
      setIsDrawing(false);
      // Normalize bounds
      let w = textInput.w;
      let h = textInput.h;
      ...
      // Default sizes if just clicked
      if (w < 20) w = 200;
      if (h < 20) h = Math.max(30, textSettings.fontSize * 1.5);

      setTextInput({x, y, w, h, text: '', isCreating: false});
      return;
    }
  ...
  return (
    <div className="relative shadow-md bg-white" style={{ width, height }} onContextMenu={(e) => e.preventDefault()}>
      ...
      {textInput && (
        <div style={{
          position: 'absolute',
          left: textInput.w < 0 ? textInput.x + textInput.w : textInput.x,
          top: textInput.h < 0 ? textInput.y + textInput.h : textInput.y,
          width: Math.abs(textInput.w),
          height: Math.abs(textInput.h),
          border: '1px dashed #0078d7',
          zIndex: 50,
          pointerEvents: textInput.isCreating ? 'none' : 'auto'
        }}>
          {!textInput.isCreating && (
            <>
              {/* Draggable header / border */}
              <div 
                className="absolute inset-0 border-[2px] border-transparent pointer-events-none"
              >
              </div>
              ...
              {[
                ...
                { id: 'n', cursor: 'ns-resize', top: -6, left: '50%', transform: 'translateX(-50%)' },
                { id: 'ne', cursor: 'nesw-resize', top: -6, right: -6 },
                { id: 'e', cursor: 'ew-resize', top: '50%', right: -6, transform: 'translateY(-50%)' },
                { id: 'se', cursor: 'nwse-resize', bottom: -6, right: -6 },
                { id: 's', cursor: 'ns-resize', bottom: -6, left: '50%', transform: 'translateX(-50%)' },
                { id: 'sw', cursor: 'nesw-resize', bottom: -6, left: -6 },
                { id: 'w', cursor: 'ew-resize', top: '50%', left: -6, transform: 'translateY(-50%)' },
              ].map(handle => (
                <div
                  key={handle.id}
                  ...
                  style={{
                    ...
                    ...(handle.transform !== undefined ? { transform: handle.transform } : {}),
                    zIndex: 70
                  }}
                  onPointerDown={(e) => {
                    e.stopPropagation();
                    e.currentTarget.setPointerCapture(e.pointerId);
                    setTextInput(prev => prev ? { ...prev, isResizing: handle.id } : null);
                  }}
                  onPointerMove={(e) => {
                    if (textInput.isResizing === handle.id) {
                       const canvas = canvasRef.current;
                       let dx = e.movementX;
                       let dy = e.movementY;
                       if (canvas) {
                       ...
                       setTextInput(prev => {
                         if (!prev) return prev;
                         let { x, y, w, h } = prev;
                         if (handle.id.includes('w')) { const nx = x + dx; const nw = w - dx; if (nw > 20) { x = nx; w = nw; } }
                         if (handle.id.includes('e')) { if (w + dx > 20) w += dx; }
                         if (handle.id.includes('n')) { const ny = y + dy; const nh = h - dy; if (nh > 20) { y = ny; h = nh; } }
                         if (handle.id.includes('s')) { if (h + dy > 20) h += dy; }
                         return { ...prev, x, y, w, h };
                       });
                    }
                  }}
                  onPointerUp={(e) => {
                    e.currentTarget.releasePointerCapture(e.pointerId);
                    setTextInput(prev => prev ? { ...prev, isResizing: null } : null);
                  }}
                />
              ))}
              
              {/* Text settings floating toolbar */}
              <div 
                className="absolute text-gray-800 left-0 bg-[#f0f0f0] p-1 border border-gray-400 rounded shadow flex items-center gap-1 z-[60]"
                style={{top: textInput.y < 50 ? 'calc(100% + 5px)' : '-45px'}}
              ...
              >
                <div 
                  ...
                  onPointerDown={(e) => {
                    e.stopPropagation();
                    e.currentTarget.setPointerCapture(e.pointerId);
                    setTextInput(prev => prev ? { ...prev, isMoving: true } : null);
                  }}
                  onPointerMove={(e) => {
                    if (textInput.isMoving) {
                       const canvas = canvasRef.current;
                       let dx = e.movementX;
                       let dy = e.movementY;
                       if (canvas) {
                  ...
                  onPointerUp={(e) => {
                    e.currentTarget.releasePointerCapture(e.pointerId);
                    setTextInput(prev => prev ? { ...prev, isMoving: false } : null);
                  }}
                >
                ...
                <select 
                  className="bg-white border border-gray-300 text-sm px-1 py-0.5 w-32"
                  value={textSettings.fontFamily}
                  onChange={e => setTextSettings(s => ({...s, fontFamily: e.target.value}))}
                >
                  {systemFonts.map(f => (
                    <option key={f.family} value={f.family} style={{fontFamily: f.family}}>{f.fullName}</option>
                  ))}
                </select>
                <select 
                  className="bg-white border border-gray-300 text-sm px-1 py-0.5 w-16"
                  value={textSettings.fontSize}
                  onChange={e => setTextSettings(s => ({...s, fontSize: parseInt(e.target.value)}))}
                >
                ...
                <button 
                  className={`w-6 h-6 flex items-center justify-center font-bold ${textSettings.bold ? 'bg-blue-200' : 'hover:bg-gray-200'}`}
                  onClick={() => setTextSettings(s => ({...s, bold: !s.bold}))}
                >B</button>
                <button 
                  className={`w-6 h-6 flex items-center justify-center italic ${textSettings.italic ? 'bg-blue-200' : 'hover:bg-gray-200'}`}
                  onClick={() => setTextSettings(s => ({...s, italic: !s.italic}))}
                >I</button>
                <button 
                  className={`w-6 h-6 flex items-center justify-center underline ${textSettings.underline ? 'bg-blue-200' : 'hover:bg-gray-200'}`}
                  onClick={() => setTextSettings(s => ({...s, underline: !s.underline}))}
                >U</button>
                <button 
                  className={`w-6 h-6 flex items-center justify-center line-through ${textSettings.strikethrough ? 'bg-blue-200' : 'hover:bg-gray-200'}`}
                  onClick={() => setTextSettings(s => ({...s, strikethrough: !s.strikethrough}))}
                >S</button>
              </div>
