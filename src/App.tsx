...
export default function App() {
  ...
  useEffect(() => {
    if (window.electronAPI) {
      ...
      window.electronAPI.getInitialFile().then((file: any) => {
        if (file) {
          setCurrentFile(file);
          const img = new Image();
          img.onload = () => {
  ...
  }, [hasUnsavedChanges, currentFile]); // Keep this effect separate for cleanup
  ...
  return (
    <div className="flex flex-col h-screen bg-[#202020] text-gray-300 font-sans select-none overflow-auto">
      ...
      >
        ...
        <div 
          className="absolute left-4 top-1/2 -translate-y-1/2 w-8 h-64 bg-[#2d2d2d] rounded-full border border-white/10 flex flex-col items-center py-4 shadow-lg z-10"
          onWheel={(e) => {
            e.stopPropagation();
            const delta = e.deltaY > 0 ? -1 : 1;
            setStrokeSize(prev => Math.max(1, Math.min(50, prev + delta)));
          }}
        >
           ...
           <div className="flex-1 w-full relative flex justify-center">
              <input 
                ...
                className="w-full h-full bg-transparent cursor-pointer m-0 p-0"
              />
           </div>
          ...
          >
            ...
            >
              <DrawingCanvas
                ...
                onColorPick={(color, isSecondary) => {
                  if (isSecondary) {
                    ...
                    if (activeColorSlot === 2) setSecondaryColor(color);
              ...
              <div 
                ...
                onMouseDown={(e) => { e.preventDefault(); window.dispatchEvent(new Event('commit-selection')); setIsResizingCanvas('e'); setResizeStart({ x: e.clientX, y: e.clientY, w: canvasSize.width, h: canvasSize.height }); }}
              ></div>
              <div 
                className="absolute top-1/2 -left-1.5 w-3 h-3 bg-white border border-gray-400 cursor-w-resize -translate-y-1/2"
                onMouseDown={(e) => { e.preventDefault(); window.dispatchEvent(new Event('commit-selection')); setIsResizingCanvas('w'); setResizeStart({ x: e.clientX, y: e.clientY, w: canvasSize.width, h: canvasSize.height }); }}
              ></div>
              <div 
                className="absolute -top-1.5 left-1/2 w-3 h-3 bg-white border border-gray-400 cursor-n-resize -translate-x-1/2"
                onMouseDown={(e) => { e.preventDefault(); window.dispatchEvent(new Event('commit-selection')); setIsResizingCanvas('n'); setResizeStart({ x: e.clientX, y: e.clientY, w: canvasSize.width, h: canvasSize.height }); }}
              ></div>
              <div 
                className="absolute -bottom-1.5 left-1/2 w-3 h-3 bg-white border border-gray-400 cursor-s-resize -translate-x-1/2"
                onMouseDown={(e) => { e.preventDefault(); window.dispatchEvent(new Event('commit-selection')); setIsResizingCanvas('s'); setResizeStart({ x: e.clientX, y: e.clientY, w: canvasSize.width, h: canvasSize.height }); }}
              ></div>
              <div 
                className="absolute -top-1.5 -left-1.5 w-3 h-3 bg-white border border-gray-400 cursor-nw-resize"
                onMouseDown={(e) => { e.preventDefault(); window.dispatchEvent(new Event('commit-selection')); setIsResizingCanvas('nw'); setResizeStart({ x: e.clientX, y: e.clientY, w: canvasSize.width, h: canvasSize.height }); }}
              ></div>
              <div 
                className="absolute -top-1.5 -right-1.5 w-3 h-3 bg-white border border-gray-400 cursor-ne-resize"
                onMouseDown={(e) => { e.preventDefault(); window.dispatchEvent(new Event('commit-selection')); setIsResizingCanvas('ne'); setResizeStart({ x: e.clientX, y: e.clientY, w: canvasSize.width, h: canvasSize.height }); }}
              ></div>
              <div 
                className="absolute -bottom-1.5 -left-1.5 w-3 h-3 bg-white border border-gray-400 cursor-sw-resize"
                onMouseDown={(e) => { e.preventDefault(); window.dispatchEvent(new Event('commit-selection')); setIsResizingCanvas('sw'); setResizeStart({ x: e.clientX, y: e.clientY, w: canvasSize.width, h: canvasSize.height }); }}
              ></div>
              <div 
                className="absolute -bottom-1.5 -right-1.5 w-3 h-3 bg-white border border-gray-400 cursor-se-resize"
                onMouseDown={(e) => { e.preventDefault(); window.dispatchEvent(new Event('commit-selection')); setIsResizingCanvas('se'); setResizeStart({ x: e.clientX, y: e.clientY, w: canvasSize.width, h: canvasSize.height }); }}
              ></div>
            </div>
          </div>
        </div>
